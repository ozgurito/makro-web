...
export default function AdminLogin() {
  ...
  // Logout param kontrolü
  useEffect(() => {
    if (searchParams.get('logout') === '1') {
      setLoggedOut(true)
      // URL'den logout param'ı temizle
      router.replace('/admin/login')
    }
  }, [searchParams, router])

  // Başarı animasyonu bittikten sonra yönlendir
  useEffect(() => {
    if (!success) return
    const t = setTimeout(() => {
      router.push('/admin')
  ...
  return (
    <>
      ...
      <div style={{
        ...
        background: 'linear-gradient(135deg, #F7F8FA 0%, #EEF2F8 100%)',
      }}>
        <div
          ...
          style={{
            ...
            background: 'white',
            borderRadius: 24,
            boxShadow: '0 20px 60px rgba(15,34,64,0.12), 0 4px 16px rgba(15,34,64,0.06)',
            overflow: 'hidden',
          }}
        >
          {/* Üst başlık şeridi */}
          <div style={{
            background: 'linear-gradient(135deg, #0F2240 0%, #1A3358 100%)',
            ...
            <div style={{
              ...
              margin: '0 auto 16px',
              boxShadow: '0 8px 24px rgba(245,124,40,0.4)',
            }}>
              <span style={{ fontFamily: 'var(--font-heading)', fontSize: 32, fontWeight: 900, color: 'white' }}>M</span>
            </div>
            <div style={{ fontSize: 11, fontWeight: 700, color: 'rgba(255,255,255,0.5)', letterSpacing: '.12em', textTransform: 'uppercase', marginBottom: 6 }}>
              Admin Paneli
            </div>
            <h1 style={{ fontFamily: 'var(--font-heading)', fontSize: 26, fontWeight: 800, color: 'white', margin: 0 }}>
              Makro İş Elbiseleri
            </h1>
          </div>
              ...
              <div style={{ textAlign: 'center', padding: '16px 0' }}>
                ...
                <div style={{ marginTop: 20, height: 3, borderRadius: 2, background: '#E5E7EB', overflow: 'hidden' }}>
                  <div style={{
                    ...
                    animation: 'progressBar 1.1s ease-in-out forwards',
                  }}/>
                </div>
                <style>{`
              ...
              <form onSubmit={handleSubmit}>
                <div style={{ marginBottom: 20 }}>
                  <label style={{
                    ...
                    color: '#6B7280', letterSpacing: '.05em', textTransform: 'uppercase',
                    marginBottom: 8,
                  }}>
                    Yönetici Şifresi
                  </label>
                ...
                <button
                  ...
                  style={{
                    width: '100%', height: 50,
                    background: loading ? '#6B7280' : 'linear-gradient(135deg, #0F2240 0%, #1A3358 100%)',
                    color: 'white', border: 'none', borderRadius: 12,
                    fontWeight: 800, fontSize: 15, cursor: loading ? 'not-allowed' : 'pointer',
                    transition: 'all .2s', display: 'flex', alignItems: 'center',
                    justifyContent: 'center', gap: 8,
                    boxShadow: loading ? 'none' : '0 4px 16px rgba(15,34,64,0.25)',
                  }}
                >
                  {loading ? (
                    <>
